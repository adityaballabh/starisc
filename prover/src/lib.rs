...
pub(crate) const LT_BITS_CON_BASE: usize = ASSERT_EQ_CON + 1; // boolean constraints for diff (64 bits)
pub(crate) const LT_RES_BOOL_CON: usize = LT_BITS_CON_BASE + NUM_LT_BITS; // res must be in {0, 1}
pub(crate) const LT_DIFF_CON: usize = LT_RES_BOOL_CON + 1; // ensure bit columns represent the actual diff
pub(crate) const NUM_LT_CONSTRAINTS: usize = NUM_LT_BITS + 2;
